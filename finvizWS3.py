...
def analyze_sentiment(headlines):
    ...
    # Add a sentiment score for each headline
    for headline in headlines:
        # Get VADER sentiment score
        vader_sentiment_score = sid.polarity_scores(headline[1])['compound']
        
        # Get TextBlob sentiment score
        textblob_sentiment_score = TextBlob(headline[1]).sentiment.polarity
        
        # Average the two sentiment scores
        combined_sentiment_score = (vader_sentiment_score + textblob_sentiment_score) / 2
        
        # Append the combined sentiment score to the headline
        headline.append(combined_sentiment_score)
    
    return headlines

def save_to_excel(headlines, ticker):
    # Convert the list of headlines to a pandas DataFrame
    df = pd.DataFrame(headlines, columns=['DateTime', 'Headline', 'Source', 'Sentiment'])
